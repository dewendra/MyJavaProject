...
@Service
public class DownloadUserEmailAddress {
	...
	public void download() {
		...
		logger.debug("Donwloading user email address");
...
// "label",  "mobileCode", "countryId", "stateId", "cityId",
		try {
			...
			for (String runnerIdRaw : runnerIds) {
				...
				logger.debug("Fetchting email for runnerId {}",runnerId);
				...
				if (response != null) {
					...
					if (root.path("status").asBoolean()) {
						...
						csvRows.add(new String[]{
								...
								/*
								 * data.path("lastName").asText(), data.path("mobile").asText(),
								 * data.path("address1").asText(), data.path("address2").asText(),
								 * data.path("landmark").asText(), data.path("country").asText(),
								 * data.path("state").asText(), data.path("city").asText(),
...
//								data.path("cityId").asText(),
	...
	private  String fetchRunnerAddress(String runnerId) {
		try {
			URL url = new URL(config.getemailaddress_url() + runnerId);
